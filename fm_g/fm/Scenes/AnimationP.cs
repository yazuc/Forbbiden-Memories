...
namespace fm{
	...
	public partial class AnimationP : Node
	{
		...
		public async Task AnimaFusaoTeste(MaoJogador maoJogador, FusionResult resultado)
		{
			maoJogador.STOP = true;
			
			// 1. Validação: Precisamos de cartas para animar
			if (resultado.CardsUsed == null || resultado.CardsUsed.Count == 0) return;

			var viewport = GetViewport();
			Vector2 screenCenter = viewport.GetVisibleRect().Size / 2f;
			
			// 2. CRIAÇÃO DAS INSTÂNCIAS USANDO O SEU MÉTODO
			// Criamos uma lista local apenas para controlar as referências visuais durante a animação
			List<CardUi> cartasVisuais = new List<CardUi>();
			
			foreach (var cardData in resultado.CardsUsed)
			{
				// 🔥 Chamada direta ao seu método que você postou
				// Ele já faz: AddChild, Set Size, DisplayCard e AtualizarNumerosFusao
				var instancia = maoJogador.CriarCartaFusao(cardData);
				
				if (instancia != null)
				{
					instancia.EscondeLabel(); 
					cartasVisuais.Add(instancia);
				}
			}

			if (cartasVisuais.Count < 2) return;

			// 3. CONFIGURAÇÃO DE LAYOUT
			var cartaPrincipal = cartasVisuais[0];
			float sideOffset = 250f; 
			float stackOffset = 30f;  

			// Centralização
			Vector2 halfSize = new Vector2(140f, 213f) / 2f; // Usando o size fixo do seu CriarCartaFusao
			Vector2 targetGlobalPos = screenCenter - halfSize;

			// 4. POSICIONAMENTO INICIAL
			List<Task> tarefasIniciais = new List<Task>();
			
			// Main vai para a Esquerda
			tarefasIniciais.Add(MoverParaPosicao(cartaPrincipal, targetGlobalPos + new Vector2(-sideOffset, 0), 0.2f));

			// O resto vira uma pilha na Direita
			for (int i = 1; i < cartasVisuais.Count; i++)
			{
				Vector2 posPilha = targetGlobalPos + new Vector2(sideOffset + (i * stackOffset), 0);
				tarefasIniciais.Add(MoverParaPosicao(cartasVisuais[i], posPilha, 0.2f));
			}

			await Task.WhenAll(tarefasIniciais);
			await Task.Delay(100); 
								
			// 5. LOOP DE STEPS (A "mágica" acontece aqui)
			for (int i = 0; i < resultado.Steps.Count; i++)
			{
				var step = resultado.Steps[i];
				var cartaSacrificio = cartasVisuais[i + 1];

				// Traz o sacrifício da pilha para o "centro de colisão"
				await MoverParaPosicao(cartaSacrificio, targetGlobalPos + new Vector2(sideOffset, 0), 0.15f);
				await Task.Delay(100);
				...
				// 7. ATUALIZAÇÃO VISUAL PÓS-STEP
				// Atualizamos a carta principal com o resultado do Step atual
				cartaPrincipal.DisplayCard(step.ResultCard, "1");
				cartaPrincipal.carta = step.ResultCard;

				Vector2 posFinal = cartaPrincipal.GlobalPosition;
				Reparentar(cartaPrincipal, this);
				cartaPrincipal.GlobalPosition = posFinal;
				cartaPrincipal.RotationDegrees = 0;

				pivot.QueueFree();
				cartaSacrificio.QueueFree(); // Remove a carta consumida da cena

				// Se houver mais passos, a carta principal volta para a esquerda para o próximo choque
				if (i < resultado.Steps.Count - 1)
				{
					await MoverParaPosicao(cartaPrincipal, targetGlobalPos + new Vector2(-sideOffset, 0), 0.2f);
					await Task.Delay(150);
				}
			}

			// FINALIZAÇÃO
			await MoverParaPosicao(cartaPrincipal, targetGlobalPos, 0.3f);
			cartaPrincipal.EscondeLabel(); // Mostra ATK/DEF da carta final
